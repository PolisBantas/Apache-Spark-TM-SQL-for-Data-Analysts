...
--TODO
create table if not exists energy using json options (
...
select
  ...
  time
from
  (SELECT * , FILTER(co2Level, level -> level > 1400) as highCO2 FROM DCDevices)
where
 size(highCO2) >0
order by
  deviceId,
...
--TODO
create table if not exists DCDevicesNew using parquet partitioned by (p_deviceId) as
select
  deviceType,
  batteryLevel,
  co2Level,
