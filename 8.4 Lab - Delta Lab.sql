...
-- TODO
create table if not exists health_tracker_data_2020 using json options (
...
-- TODO
CREATE OR REPLACE TABLE health_tracker_silver1 
USING DELTA
PARTITIONED BY (p_device_id)
LOCATION "/health_tracker/silver1" AS (
SELECT
  value.name,
  value.heartrate,
...
-- TODO
DROP TABLE IF EXISTS health_tracker_silver1;
CREATE TABLE health_tracker_silver1
USING DELTA
LOCATION "/health_tracker/silver1"
...
--TODO
create
or replace temporary view broken_readings as(
  select
    count(*) as broken_readings,
    dte
  from
    health_tracker_silver1
  where
...
CREATE TABLE health_tracker_data_2020_late USING json OPTIONS (
...
--TODO
MERGE INTO health_tracker_silver1                           
USING upserts

ON health_tracker_silver1.time = upserts.time AND        
   health_tracker_silver1.p_device_id = upserts.p_device_id  
   
WHEN MATCHED THEN                                           
  UPDATE SET
  health_tracker_silver1.heartrate = upserts.heartrate   
...
--TODO
DROP TABLE IF EXISTS health_tracker_gold1;              
