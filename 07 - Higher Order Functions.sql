...
select
  firstName,
  lastName,
  transform(expenses, c -> round(c.charges * 0.625, 2)) as interest
from
  finances;
...
select
  ...
  exists(
    expenses,
    c -> to_date(c.paymentDue) > to_date(c.lastPayment)
  ) as lateFee
from
  finances;
...
--TODO   Answer 3
create
or replace temporary view q3Results as
select
  firstName,
  lastName,
  allCharges,
  reduce(
    allCharges,
    cast(0 as double), (charge, acc) -> charge + acc) as totalDollars,
    reduce(
      allCharges,
      cast(
        0 as double),
        (charge, acc) -> charge + acc,
        acc -> round(acc * 107.26, 2)
      ) as totalYen
      from
        charges;
